  ...
  {:else}
    ...
    {#if inviteOpen}
      <section class="surface-panel overflow-hidden">
        <div
          class="border-b border-border/60 bg-primary/[0.04] px-6 py-5 sm:px-7"
        >
          <div class="flex items-start justify-between gap-5">
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <h2 class="mt-1 font-heading text-lg font-bold text-foreground">
                  Invite someone to {$workspaceName}
                </h2>
                <p class="mt-1 max-w-xl text-sm text-muted-foreground">
                  Send a secure invitation and choose the role they will use in
                  this workspace.
                </p>
              </div>
            </div>
        ...
        <form
          class="grid gap-5 p-6 sm:grid-cols-[minmax(0,1fr)_180px_auto] sm:items-end sm:p-7"
        ...
        {/if}
        <div
          class="flex items-center gap-2 border-t border-border/50 px-6 py-3.5 text-xs text-muted-foreground sm:px-7"
        >
      ...
      <div class="mt-5 space-y-3">
        {#each filteredMembers as member (member.id)}
          <div
            ...
            onkeydown={(event) =>
              event.key === "Enter" && openMember(member.id)}
          >
            <div
            ...
            {#if member.role === "STAFF" && member.status === "Active" && ($authStore.user?.role === "OWNER" || $authStore.user?.role === "ADMIN")}
              <div
                class="flex w-full flex-wrap items-center gap-2 border-t border-border/50 pt-3 sm:ml-auto sm:w-auto sm:border-t-0 sm:pt-0"
              >
                <span
                  class="inline-flex items-center gap-1.5 text-xs font-semibold text-muted-foreground"
                >
                ...
                {#each $installedExtensionIds as extension}
                  {@const granted = ($staffAccessMap[extension] ?? []).includes(
                    member.id,
                  )}
                  <button
                    type="button"
                ...
                {#if $installedExtensionIds.length === 0}
                  <span class="text-xs text-muted-foreground"
                    >No installed extensions</span
                  >
                {/if}
              </div>
            {:else if member.role === "OWNER" || member.role === "ADMIN"}
              <span
                class="flex w-full items-center gap-1.5 border-t border-border/50 pt-3 text-xs font-semibold text-primary sm:ml-auto sm:w-auto sm:border-t-0 sm:pt-0"
              >
